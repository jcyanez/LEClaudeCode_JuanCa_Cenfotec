...
- Este directorio vive como carpeta normal (`Week2/fretPath-estudiantes/`)
  dentro del repo único del curso (`jcyanez/LEClaudeCode_JuanCa_Cenfotec`),
  rama `main`, junto a `Week1/` y las semanas siguientes. Ya no es un repo
  git anidado.
- Historial detallado del caso (commit a commit, sobre la línea base
  `f332319` del instructor) preservado íntegro en la rama
  `week2-fretpath-caso2` del mismo repo:
  `8d0b3bd` (fix + regresión/contrato) → `cd891f1` (nodesAtRisk + bordes) →
  `f14655b` (bitácora) → `298a481` (invariantes temporales) → `d0a5b43`
  (entregables) → `a136a06` (hashes tras reescritura de email) → `3cf4834`
  (enlace de entrega) → `b070e3b`/`198e261` (excluir material personal del
  entregable). Esos hashes existen solo en esa rama; en `main` el contenido
  llega como commit(s) nuevos al incorporar la carpeta.
- Entrega: único enlace enviado al profesor —
  https://github.com/jcyanez/LEClaudeCode_JuanCa_Cenfotec (rama `main`,
  carpeta `Week2/fretPath-estudiantes/`).
