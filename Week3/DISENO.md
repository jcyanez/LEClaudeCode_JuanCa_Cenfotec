...
| Lenguaje y marco del servicio | TypeScript full-stack / Python + Django / C# + ASP.NET Core | **TypeScript**: Node.js con Fastify en el servidor; React en la PWA | Un solo lenguaje en todo el proyecto y tipos compartidos entre dominio y pantallas. Decidido por el usuario en T0. *(La elección original incluía `@carbon/react` como librería de componentes; se revirtió el 15/08 — ver la fila «Sistema visual de las pantallas».)* |
...
| Sistema visual de las pantallas | Librería de Carbon como dependencia / Carbon solo como inspiración / **sistema propio guiado por la skill `ui-ux-pro-max`** | **Sistema propio de tokens y componentes**, derivado de la entrada `Theater/Cinema` de la skill: «Dark Mode (OLED) + Motion-Driven», paleta «dramatic dark + spotlight gold» y tipografía Inter | Carbon es un sistema de productividad empresarial: sirve para las pantallas de trabajo, pero no para la cara que ve quien compra una entrada de cine. La skill tiene una recomendación específica para salas de cine, y un conjunto propio y pequeño de componentes evita arrastrar una librería entera por trece controles. **Cambio de tecnología decidido por el usuario el 15/08**, revierte la decisión del 11/08. **Completado el 15/08**: `@carbon/react` y `@ibm/plex` ya no son dependencias del proyecto |
| Estilo de las pantallas de trabajo | Un solo estilo para todo el sistema / **Swiss Modernism 2.0 + Data-Dense Dashboard** para taquilla, puerta y administración | **Retícula estricta de 12 columnas, espaciado sobre una unidad de 8 px, tipografía Inter y un solo acento**, con la densidad de un tablero de datos (relleno de 12 px, filas de 36 px, encabezados fijos, tablas que se desplazan en su caja) | Los dos estilos vienen de la skill y se complementan: el primero da el carácter y una jerarquía que se lee de un vistazo (declara WCAG AAA); el segundo, la densidad que una ventanilla necesita. El reparto de columnas sigue el uso real: en la puerta el veredicto ocupa siete de doce porque leer «pasa / no pasa» a un metro importa más que el formulario, y en taquilla el panel de cobro queda fijo mientras se eligen butacas |
...
| Forma de la cartelera pública | Una tarjeta por función / **una tarjeta por película, con sus horarios** | **Agrupada por película**, con los horarios del día elegido dentro de cada tarjeta | Una tarjeta por función repetía la misma película tantas veces como horarios tuviera: esa es la forma de la tabla, no la de la decisión. Quien entra elige primero qué ver y después a qué hora. No cambia ningún dato: `GET /api/cartelera` devuelve lo mismo y el agrupado ocurre en Entrada |
| Dónde viven el póster y el género | En la base y la API, como columnas de `pelicula` / **tabla fija en la interfaz, por título** | **Tabla fija en la interfaz** (`entrada-cliente/src/componentes/publico/posters.ts`) | **Es deuda elegida, no un descuido.** Lo correcto es que fueran columnas de `pelicula`, para que la dueña cargue el póster desde su pantalla (`RF-1`); se prefirió la vía corta porque no toca la base, ni la API, ni el modelo de `RN-4`. Lo que cuesta: agregar una película nueva exige editar código y desplegar, y si alguien la renombra desde administración pierde el póster. Decidido por el usuario el 15/08 |
| Qué pasa si una película no tiene póster | Ocultar la tarjeta / imagen genérica / **respaldo con leyenda** | **Respaldo del mismo tamaño con la leyenda «Sin póster»** | La película se sigue pudiendo comprar: lo que falta es la imagen, no el dato. Ocupa el mismo espacio que el cartel, así que la tarjeta no cambia de alto, y **nunca aparece el icono roto del navegador** |
| Cómo se llega a los horarios en la cartelera | Siempre a la vista / **detrás del cartel** | **El cartel es un botón que despliega los horarios de esa película** | Los horarios a la vista dejaban al cartel sin nada que hacer, con un rótulo «Ver horarios» que prometía algo ya ocurrido. Cuesta un toque más en la tarea principal, compensado porque la función más próxima conserva su camino directo desde el encabezado. El disparador es un `button` con `aria-expanded` y rótulo siempre visible: en un teléfono no hay puntero. Decidido por el usuario el 15/08 |
